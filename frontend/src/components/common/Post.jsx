...
const Post = ({ post }) => {
  ...
  const { data: authUser = {} } = useQuery({
    queryKey: ["authUser"],
    // Add a placeholder or cache the data if necessary
  });
        ...
        oldData.map((p) =>
          p._id === post?._id
            ? {
                ...
                comments: [
                  ...p.comments,
                  { text: comment, user: authUser._id },
                ],
              }
            : p
        )
        ...
        <div className="flex justify-between mt-3">
          <div className="flex gap-4 items-center w-2/3 justify-between">
            ...
            >
              ...
              <span
                className={`text-sm ${isLiked ? "text-pink-500" : "text-slate-500"} group-hover:text-pink-500`}
              >
                {post.likes.length}
              </span>
            </div>
